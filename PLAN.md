...
## P3-05. 보고서 품질 대폭 개선 — 미착수 (최우선)
현재 문제:
- 주간 보고서 프롬프트(REPORT_PROMPT_WEEKLY) 없음 → daily 폴백 → 341자
- PDF 차트: matplotlib silent fail → 차트 없는 PDF
- PDF 경로: 개발PC 맥 경로 → Docker에서 접근 불가
- AI 텍스트는 훌륭 (16섹션, 4000자+) → PDF 변환 단계에서 품질 하락

필요한 작업:
- [ ] REPORT_PROMPT_WEEKLY 작성 (한 주간 변화 추이, 경쟁자 동향, 전략 조정)
- [ ] PDF 차트 생성 Docker에서 작동 확인 (matplotlib + NanumGothic)
- [ ] PDF 경로를 Docker 볼륨으로 변경
- [ ] 일일 보고서 PDF: 표지 + 차트 + 16섹션 완전체
- [ ] 주간 보고서 PDF: 주간 추이 차트 + SWOT 변화 + 전략 평가
- [ ] 보고서 다운로드 기능 프론트엔드에서 확인
