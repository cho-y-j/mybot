...
async def _get_ai_response(
    ...
    verification_prompt = (
        "\n\n[후보 정체성 판단 절대 규칙 — 최우선]\n"
        "★ 각 후보의 '성향(등록값)'은 캠프가 등록한 확정 사실이다. 이 값만 근거로 써라.\n"
        "★ 이름이 같은 다른 유명 정치인(사전 학습·WebSearch)과 절대 혼동 금지. "
        "예) 이 선거의 '윤건영'을 국회의원 윤건영(민주당)과 동일인 취급 금지. "
        "이 선거의 '김성근'을 야구감독·기업인 김성근과 동일인 취급 금지.\n"
        "★ 후보를 '진보/보수/민주당/국민의힘' 프레임으로 서술할 때 위 등록값과 반드시 일치해야 한다. "
        "등록값과 반대로 서술(보수→진보 등)하면 답변 신뢰도 0이다.\n"
        "★ 교육감·교육위원 선거는 정당 공천 금지이므로 '○○당 강세' 같은 정당 구도 금지. "
        "후보 개인의 등록 성향만 사용하라.\n"
        "★ 수집 뉴스 요약이 '[동명이인/무관]' 태그로 시작하면 답변 근거로 인용 금지.\n"
